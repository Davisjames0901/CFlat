...
namespace Digman.IO.CFlat.Tests
{
  public static class KeywordTest
  {
    ...
    {
      var keywordNames = Enum.GetNames(typeof(Keywords)).ToList();
      var keywordValues = (int[])Enum.GetValues(typeof(Keywords));
      ...
      foreach (var a in EnumsToTest)
      {
        ...
        for(var i = 0; i < values.Length; i++)
        {
          ...
          {
            ConsoleHelpers.PrintError($"Enum {names[i]} in type {a} was not found in {typeof(Keywords)}");
            issueFound = true;
          }
        }
      }
